...
var https = getXMLHTTPRequest1();
function showData2() {
var myurls = 'getdata2.php';
var ones=document.getElementById('sems');
var sems=ones.options[ones.selectedIndex].value;
var modurls = myurls+"?q="+sems;
https.open("GET", modurls, true);
https.onreadystatechange = useHttpResponse1;
https.send(null);
}
function useHttpResponse1() {
if (https.readyState == 4) {
if(https.status == 200) {
var mytexts = https.responseText;
document.getElementById('data2').innerHTML = mytexts;
}
} else {
...
#myForm input:focus:invalid, #myForm textarea:focus:invalid { 
  ...
  <body class="fancy" onLoad=""showData()>
            ...
            <div>
              <ul>
                <li><?php
					...
					{
					echo "<h2>HELLO, ".$row['FIRSTNAME']." ".$row['LASTNAME']."<br></h2>";
...
<li><h3>Address</h3><br>
<label>Line 1:</label><input type="text" name="line1" />
<label>Line 2:</label><input type="text" name="line2" /><br>
<label>P.O.:</label><input type="text" name="po" />
<label>Pin Code:</label><input type="text" name="pin" pattern="[0-9]{6}"/><br>
...
<button class="submit" type="submit">Submit Form</button>
            ...
            <div>
              <ul>
                <li><?php
					...
					(SELECT CODE,TID FROM RESULT WHERE SID='$id' AND SEM=(SELECT SEM FROM STUDENT WHERE ID='$id')) AS P LEFT JOIN
					SUBJECT AS S ON P.CODE=S.CODE LEFT JOIN (SELECT ID,FIRSTNAME,MIDNAME,LASTNAME FROM TEACHER) AS T ON T.ID=P.TID
					");
            ...
            <div>
				...
				<?php
					...
					echo "<label>Semester:</label> <select name='sems' id='sems'>";
					for($i=1;$i<$sem;$i++)
					{
					echo "<option value='".$i."'>".$i."</option>";
					
					}
					echo "<option value='9'>Overall</option>";
					echo "</select>";
						
					echo "<button class='submit' onClick='showData2()'>Submit</button>";
